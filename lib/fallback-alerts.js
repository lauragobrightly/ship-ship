...
export function createFallbackAlerter({
  ...
  function describe({bucket, kind, quoteResult, groupSubtotal, held}) {
    ...
    const poolTotal = Number.isFinite(quoteResult?.poolCents)
      ...
      : 'unknown';
    const tail = held
      ? `No corrected quote arrived for this destination within ${Math.round(holdMs / 1000)}s, so the customer may have checked out at $0.`
      : 'Customer was charged $0 to prevent a duplicate warehouse fee. This shape is not a race; investigate the checkout path.';
    return {
